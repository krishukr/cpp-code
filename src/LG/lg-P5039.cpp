...
int main() {
    ...
    int n = read<int>(), m = read<int>(), l = read<int>();
    ...
    flow->set_st(q[l].x, q[l].y);

    for (int i = 1; i <= m; i++) {
        if (q[i].z <= q[l].z and i != l) {
            flow->create(q[i].x, q[i].y, q[l].z - q[i].z + 1);
            flow->create(q[i].y, q[i].x, 0);
            flow->create(q[i].y, q[i].x, q[l].z - q[i].z + 1);
            flow->create(q[i].x, q[i].y, 0);
        }
    }
