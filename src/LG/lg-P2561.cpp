...
struct BigInt {
    ...
    explicit operator bool() const { return (bool)(*this != BigInt(0)); }
};
...
int main() {
    ...
    read(n);
    int m = n * (n + 1) / 2;
    BigInt t = quick_pow(BigInt(2), m);
    int c = 0;
    for (int i = 1; i <= n; i++) {
        c += (i + 1) / 2;
    }
    t += quick_pow(BigInt(2), c);

    if (n % 3 == 1) {
        t += BigInt(2) * (quick_pow(BigInt(2), (m - 1) / 3 + 1) +
                          quick_pow(BigInt(2), (m - 4) / 2 + 3));
    } else {
        t +=
            BigInt(2) * (quick_pow(BigInt(2), m / 3) + quick_pow(BigInt(2), c));
    }
    std::cout << t / 6 << '\n';
